...
namespace API.Services.Inventory
{
    public class InventoryService(ApplicationDbContext context) : IInventoryService
    {
        ...
        public async Task<bool> SaveNumberConsecInventoryAsync(string numero,string filter)
        {
            ...
            var setting = await Context.Parametros.FirstOrDefaultAsync(x => x.Module == filter);
            if (setting == null) return false;

            setting.Value1 = numero;
            await Context.SaveChangesAsync();
            return true;
        }
        public async Task<DocumentSettings> GetConfigById(string filter)
        {
            if (string.IsNullOrWhiteSpace(filter)) return new DocumentSettings();
            ...
            {
                Consec = string.IsNullOrEmpty(setting.Value1) ? 0 : Convert.ToInt32(setting.Value1),
                Prefijo = setting.Value2 ?? string.Empty,
                useSeparator = string.IsNullOrEmpty(setting.Value3) ? false : Convert.ToBoolean(setting.Value3),
                usePref = string.IsNullOrEmpty(setting.Value4) ? false : Convert.ToBoolean(setting.Value4),
                CharacterSeparator = setting.Value5 ?? string.Empty
            };
            return ds;
        }
